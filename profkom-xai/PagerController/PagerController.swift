...
open class PagerController: BaseViewController, UIPageViewControllerDataSource, UIPageViewControllerDelegate, UIScrollViewDelegate {
	...
	open var startIndex = 0
	open var centerCurrentTab: Bool = false
	open var fixFormerTabsPositions: Bool = false
	open var fixLaterTabsPosition: Bool = false
    var isPageViewControllerHidden = false {
        didSet {
            pageViewController.view.isHidden = isPageViewControllerHidden
            tabsView?.isHidden = isPageViewControllerHidden
            hairline?.isHidden = isPageViewControllerHidden
        }
    }
	...
	func defaultSetup() {
		...
		if self.tabsView == nil {
            ...
            if !isIPhone {
                self.tabsView!.contentInset = UIEdgeInsets(top: 0, left: (view.frame.width - CGFloat(tabCount) * tabWidth) / 2, bottom: 0, right: 0)
            }
        ...
        if self.hairline == nil {
            hairline = UIView(frame: CGRect(x: 0, y: 0, width: self.view.frame.width, height: 1 / UIScreen.main.scale))
            hairline!.isHidden = isPageViewControllerHidden
            hairline!.autoresizingMask = [.flexibleWidth]
            hairline!.backgroundColor = UIColor(red: 178 / 255.0, green: 178 / 255.0, blue: 178 / 255.0, alpha: 1)
            
            self.view.insertSubview(hairline!, aboveSubview: contentView)
        }
        

		// Select starting tab
		self.selectTabAtIndex(startIndex, swipe: true)
	...
	func layoutSubViews() {
        ...
        var hlFrame = self.hairline!.frame
        hlFrame.origin.y = frame.minY
        hlFrame.size.width = self.view.frame.width
        self.hairline!.frame = hlFrame
	}
